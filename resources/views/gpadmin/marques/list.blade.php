            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-3 d-flex justify-content-end">
                        <a href="{{ route('gpadmin.marquee.create') }}" class="btn btn-sm btn-outline-primary" >Add Marque</a>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped datatables">
                            <thead>
                                <tr>
                                    <th>Sr.No.</th>
                                    <th>Marquee message</th>
                                    <th>Status</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($marquee as $key => $role)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $role->message }}</td>
                                        <td>
                                            <form action="{{ route('gpadmin.marquee.updatestatus') }}" method="POST"
                                        ...
                                        <td>
                                            <a href="{{ route('gpadmin.marquee.edit', base64_encode($role->id)) }}"
                                                class="btn btn-sm btn-outline-primary">Edit</a>
                                            <form action="{{ route('gpadmin.marquee.delete') }}" method="POST"
                                                class="d-inline-block delete-form">
                                                @csrf
