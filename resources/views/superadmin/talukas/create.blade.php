...
.form-page-title .form-icon { width: 44px; height: 44px; border-radius: 11px; background: linear-gradient(135deg, #1a73e8, #0d47a1); display: flex; align-items: center; justify-content: center; color: #fff; font-size: 1.25rem; box-shadow: 0 3px 10px rgba(26,115,232,0.3); flex-shrink: 0; }
...
.form-group .form-control:focus { border-color: #1a73e8; box-shadow: 0 0 0 3px rgba(26,115,232,0.1); background: #fff; }
...
.btn-save { background: linear-gradient(135deg, #1a73e8, #0d47a1); color: #fff; border: none; border-radius: 8px; padding: 9px 24px; font-size: 0.87rem; font-weight: 700; box-shadow: 0 3px 10px rgba(26,115,232,0.3); transition: opacity 0.18s, transform 0.18s; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; }
...
<div class="form-page-header">
    <div class="form-page-title">
        ...
        <div>
            <h4>Add New Taluka</h4>
            <p>Create a new taluka under a district</p>
        </div>
    </div>
...
<div class="row justify-content-center">
    <div class="col-12 col-md-8 col-lg-6">
        <div class="form-card">
            <form action="{{ route('superadmin.talukas.store') }}" method="POST">
                @csrf
                <div class="form-card-body">
                    <div class="form-group">
                        <label>Taluka Name</label>
                        <input type="text" name="taluka_name" value="{{ old('taluka_name') }}"
                            class="form-control @error('taluka_name') is-invalid @enderror"
                    ...
                    <div class="form-group">
                        ...
                        <select name="district_id" class="form-control @error('district_id') is-invalid @enderror" required>
                            ...
                            @foreach($districts as $d)
                                <option value="{{ $d->id }}" {{ old('district_id') == $d->id ? 'selected' : '' }}>{{ $d->district_name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-footer">
                    ...
                    <button type="submit" class="btn-save"><i class="mdi mdi-content-save-outline"></i> Save Taluka</button>
                </div>
            </form>
        </div>
    </div>
